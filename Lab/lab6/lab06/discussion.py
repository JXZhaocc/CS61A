...
class Button:
    ...
    def press(self):
        ...
        self.pressed += 1
        "*** YOUR CODE HERE ***"
        if self.caps_lock.pressed % 2 == 0: # use class atrribute need to use self too
...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {c: Button(c, self.typed.append) for c in LOWERCASE_LETTERS} # Try a dictionary comprehension! # the second argument was tricky
    ...
    def type(self, word):
        ...
        "*** YOUR CODE HERE ***"
        for i in word:
            self.keys[i].press()# my previous answer: self.typed.append(self.keys[i].press())
